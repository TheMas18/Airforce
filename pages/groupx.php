    ...
    if ($ssc <= $sscOut && $hsc <= $hscOut && $hscEng <= $hscEngOut && $hscPhy <= $hscPhyOut) {
      ...
      if (($glasses == 1 && $glassesPOWER == 1) || ($glasses == 0 && $glassesPOWER == 0)) {
        if (($sscPerc >= 50 && $sscPerc <= 100) && ($hscPerc >= 50 && $hscPerc <= 100) && ($hscEngPerc >= 50 && $hscEngPerc <= 100) && ($hscPhyPerc >= 50 && $hscPhyPerc <= 100)) {
          if ($height >= 163 && ($weight >= 50 && $weight <= 70)) {
            if ($age >= 18 && $age <= 22) {
              $show_success = true;
            } else {
              $show_failure = true;
...
<body>
  ...
  <div class="container mt-3">
    <div class="bg-light p-5 rounded">
      <h1>Airforce Group X & Y Salary</h1>
      <p>Joining the Indian Air Force as Airmen for Group X and Y posts is one of the most reputable jobs for any candidate willing to join the Defence Forces of India. Lakhs of applications are received each year by the male candidates who have passed their class 12th for the Indian Air Force Group X and Y as it is one of the highly reputable job that ensures a great pay scale, job security with a fair amount of perks, benefits, and allowances. Read this article to know about the Indian Air force Group X and y salary, on profile, career growth, and perks & allowances received by the selected candidates.</p>
      <br>
      <h1>Air Force Group X & Y Salary: Know Career Growth</h1>
      <p>The Indian Air Force is recruiting airmen for Group X & Y. The Group X is for the candidates having a technical qualification, that who have completed their 10+2 with Science and Maths or Diploma holders. While Group Y is for candidates having a non-technical qualification.Here is the salary & career growth of candidates joining the Indian Air Force as Airmen in Group X and Y.</p>
